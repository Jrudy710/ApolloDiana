...
# Run the program in the terminal. Command line should be like so: "python Project2.py input.txt output.txt"

# 11/22/2023 - Original Version
# 11/23/2023 - Added the libraries so the code works on the terminal, i.e. sys methods, or command line stuff
   # and changed the necessary variables in the main method to take the info from the terminal

#import sys                                                                    # Comment this out to mess with stuff after submitted
import math
import heapq
...
# This class is an implementation of a priority queue using the heap libraries that are available in
# python for us to use upon importing. The Class allows for us to push data into the priority queue object, remove from
# the priority queue object, check if the priority queue is empty, and update the priority queue object
class PriorityQueue:
    """
...
def astarSearch(grid):                                                                                               # Method Block
   ...
   while not pQueue.isEmpty():                                                                                       # While Loop
      ...
      while (xCoordinate >= 0 and xCoordinate < len(grid)) and (yCoordinate >= 0 and yCoordinate < len(grid[0])):    # While Loop
         ...
         if arrowColor != (grid[xCoordinate][yCoordinate])[: 1]:                                                     # Checking to see if the arrow color is different from the starting arrow color
            ...
            #print(newRoute, "\n")
            pQueue.push((newPos, newRoute), (2 * heuristic) + (1 * totalCost(path)))                                             # Adds to the priorityQueue
   ...
   #print("Did the loop fail")

def main():
   
   text_file = "example.txt"                                                                                           # Sets the variable for the text file
   ...
   traversalPath = astarSearch(grid)                                                                                 # Call to method astarSearch
   
   end = timer()
   
   print(" ".join(traversalPath))
   
   print(f"Traversal method took {end - start} seconds")
   fileData = open(text_file[: -4] + "-soln.txt", "w")
   
   #fileData = open(output, "w")                                                                                      # Opens the output file to write to it
   fileData.write(" ".join(traversalPath))                                                                           # Writes the directions to the file
